...
def load_sensitivity_data(root_dir, drug_features = [], response_sources = ['CTRP']):
    ...
    # Response data
    X_key = cl_response.loc[cl_response['SOURCE'].isin(response_sources), :] #['CELL', 'DRUG', 'AAC1']]
    ...
    if np.isin('dragon_desc', drug_features):
        data_dict['X_drug_desc'] = drug_dragon_desc.loc[X_key['drug_pubchem']].dropna(axis = 1).to_numpy()
    if np.isin('dragon_ecfp', drug_features):
        data_dict['X_drug_ecfp'] = drug_dragon_ecfp.loc[X_key['drug_pubchem']].dropna(axis = 1).to_numpy()
    if np.isin('dragon_pfp', drug_features):
        data_dict['X_drug_pfp'] = drug_dragon_pfp.loc[X_key['drug_pubchem']].dropna(axis = 1).to_numpy()
    if np.isin('morderd', drug_features):
        data_dict['X_drug_mordred'] = drug_mordred.loc[X_key['DRUG']].dropna(axis = 1).to_numpy()
...
def load_old_sensitivity_data(
    ...
    if len(cell_line_filter_inclusion_list) > 0:
        cl_filter_table = pd.read_csv(cell_line_expression_root_dir + cell_line_filter_mapping_file, header = 0, index_col = 0)
        cl_filter_code = cl_filter_table.loc[cl_data_ind, cell_line_filter_mapping_column]
        cl_filter = np.logical_not(cl_filter_code.isna().to_numpy())
        cl_filter[cl_filter] = np.isin(cl_filter_code[cl_filter].to_numpy(), cell_line_filter_inclusion_list)
    ...
    if cell_line_expression_mean_cut is not None:
        cl_gene_mean_filter = cl_data.to_numpy().mean(axis = 0) > cell_line_expression_mean_cut
    else:
